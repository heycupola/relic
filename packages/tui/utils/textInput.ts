...
export function deleteWordBackward(text: string, cursorPos: number): TextCursorResult {
  ...
  const newPos = moveWordBackward(text, cursorPos);
  const newText = text.slice(0, newPos) + text.slice(cursorPos);
  return { text: newText, cursor: newPos };
}

export function deleteWordForward(text: string, cursorPos: number): TextCursorResult {
  const endPos = moveWordForward(text, cursorPos);
  return { text: text.slice(0, cursorPos) + text.slice(endPos), cursor: cursorPos };
}
