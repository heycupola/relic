...
import { BulkImportModal, CommandPaletteModal, ManageCollaboratorsModal } from "../components/modals";
...
const MOCK_SECRETS: Secret[] = [
  { id: "s1", key: "DATABASE_URL", value: "postgresql://user:pass@localhost:5432/db", type: "string", environmentId: "env1" },
  { id: "s2", key: "API_KEY", value: "sk_test_4eC39HqLyjWDarjtT1zdp7dc", type: "string", environmentId: "env1" },
  { id: "s3", key: "JWT_SECRET", value: "a-very-secret-signing-key", type: "string", environmentId: "env1", folderId: "f1" },
...
export function ProjectPage({ projectId: _projectId, projectName, projectStatus, onBack }: ProjectPageProps) {
  ...
  const [editingItem, setEditingItem] = useState<{ type: "env" | "folder"; id: string; name: string } | null>(null);
  const [confirmingDelete, setConfirmingDelete] = useState<{ type: "env" | "folder" | "secret"; id: string; name: string } | null>(null);
  ...
  const getCurrentItems = useCallback(() => {
    ...
    if (viewLevel === "environment" && selectedEnvId) {
      ...
      return [
        ...
        ...rootSecrets.map((s) => ({ type: "secret" as const, id: s.id, name: s.key, value: s.value, secretType: s.type || "string" })),
      ];
    }
    if (viewLevel === "folder" && selectedFolderId) {
      return secrets.filter((s) => s.folderId === selectedFolderId)
        .map((s) => ({ type: "secret" as const, id: s.id, name: s.key, value: s.value, secretType: s.type || "string" }));
    }
    return [];
  ...
  useEffect(() => {
    ...
    if (!trimmed) { setBulkImportCollisions([]); return; }
    ...
    try {
      ...
      if (result.valid && result.secrets.length > 0) {
        const collisions: CollisionInfo[] = result.secrets
          ...
          .map((s) => ({ key: s.key, existingSecretId: secrets.find((e) => e.key === s.key)?.id || "" }));
  ...
  const moveUp = () => {
    ...
    setSelectedIndex((prev) => {
      ...
      else if (next >= scrollOffset + PAGE_SIZE) setScrollOffset(Math.max(0, items.length - PAGE_SIZE));
      return next;
    });
  };
  ...
  const handleCreateItem = async (name: string) => {
    ...
    await runTask(`Creating ${itemType} "${name}"...`, () => new Promise((r) => setTimeout(r, 1000)));
    showSuccess(`${itemType.charAt(0).toUpperCase() + itemType.slice(1)} "${name}" created`);
    setCreatingItem(null);
  };

  const handleRenameItem = async (name: string) => {
    if (!editingItem || name === editingItem.name) { setEditingItem(null); return; }
    const itemType = editingItem.type === "env" ? "environment" : "folder";
    await runTask(`Renaming ${itemType} to "${name}"...`, () => new Promise((r) => setTimeout(r, 1000)));
  ...
  const handleDeleteItem = async () => {
    ...
    await runTask(`Deleting ${itemType} "${confirmingDelete.name}"...`, () => new Promise((r) => setTimeout(r, 1000)));
    showSuccess(`${confirmingDelete.name} deleted`);
    setConfirmingDelete(null);
  };

  const handleAddCollaborator = async (email: string) => {
    await runTask(`Adding collaborator "${email}"...`, () => new Promise((r) => setTimeout(r, 1000)));
  ...
  const getAllCommands = () => {
    const cmds = [];
    if (viewLevel === "environments") {
      cmds.push({ key: "n", description: "Create environment", category: "Create", disabled: isRestricted });
      cmds.push({ key: "u", description: "Rename environment", category: "Manage", disabled: isRestricted });
      cmds.push({ key: "esc", description: "Back to home", category: "Navigate" });
    } else if (viewLevel === "environment") {
      cmds.push({ key: "n", description: "Create folder", category: "Create", disabled: isRestricted });
      const item = items[selectedIndex];
      if (item?.type === "folder") cmds.push({ key: "u", description: "Rename folder", category: "Manage", disabled: isRestricted });
      cmds.push({ key: "⌥u", description: "Update secrets", category: "Manage", disabled: isRestricted });
      cmds.push({ key: "⌥i", description: "Import secrets", category: "Create", disabled: isRestricted });
    ...
    cmds.push({ key: "c", description: "Manage collaborators", category: "Manage", disabled: isRestricted });
    if (viewLevel !== "environments") cmds.push({ key: "v", description: showSecrets ? "Hide secrets" : "Show secrets", category: "View" });
    return cmds.sort((a, b) => ["Navigate", "Create", "Manage", "View"].indexOf(a.category) - ["Navigate", "Create", "Manage", "View"].indexOf(b.category));
  };

  const executeCommand = (cmd: { key: string }) => {
    if (isRestricted && ["n", "u", "c", "⌥i", "⌥u"].includes(cmd.key)) return;
    switch (cmd.key) {
      case "n": setCreatingItem(viewLevel === "environments" ? "env" : "folder"); break;
      case "u": {
        if (viewLevel === "environments") {
          ...
          const item = items[selectedIndex];
          if (item?.type === "folder") setEditingItem({ type: "folder", id: item.id, name: item.name });
        }
        break;
      }
      case "esc": goBack(); break;
      case "⌥i": openBulkImport("import"); break;
      case "⌥u": openBulkImport("update"); break;
      case "c": setActiveModal("manageCollaborators"); break;
      case "v": setShowSecrets((p) => !p); break;
    }
  };
  ...
  const openBulkImport = (mode: "import" | "update") => {
    if (mode === "import") {
      ...
      const secretsJson = JSON.stringify(items.filter((i) => i.type === "secret").map((i) => ({ key: i.name, value: (i as { value?: string }).value || "", type: "string", scope: "shared" })), null, 2);
  ...
  useKeyboard((key) => {
    ...
    // Bulk import modal
    if (activeModal === "bulkImport") {
      if (key.name === "escape") { setActiveModal("none"); bulkImportInput.reset(); return; }
      if (key.name === "j" && key.meta) {
        ...
        if (bulkImportFormat === "env" && content) {
          ...
          if (json && json !== "[]") { bulkImportInput.setValue(json); setBulkImportFormat("json"); }
        } else if (content) {
          const env = jsonToEnv(content);
          if (env) { bulkImportInput.setValue(env); setBulkImportFormat("env"); }
        }
        return;
      }
      if ((key.name === "i" || key.name === "o") && key.meta) {
        ...
        try {
          ...
          runTask(`Importing secrets (${action} collisions)...`, () => new Promise((r) => setTimeout(r, 1000)));
    ...
    // Navigation & actions
    if (key.name === "escape" || key.name === "backspace") { goBack(); setConfirmingDelete(null); }
    else if (key.name === "k" || key.name === "up") { moveUp(); setConfirmingDelete(null); }
    else if (key.name === "j" || key.name === "down") { moveDown(); setConfirmingDelete(null); }
    else if (key.name === "return" || key.name === "l" || key.name === "right") enter();
    else if (key.name === "d" && !isRestricted) {
      const item = items[selectedIndex];
      if (item) setConfirmingDelete({ type: item.type as "env" | "folder" | "secret", id: item.id, name: item.name });
    }
    else if (key.name === "n" && !isRestricted) setCreatingItem(viewLevel === "environments" ? "env" : viewLevel === "environment" ? "folder" : null);
    ...
    else if (key.name === "i" && key.meta && !isRestricted && viewLevel !== "environments") openBulkImport("import");
    else if (key.name === "u" && key.meta && !isRestricted && viewLevel !== "environments") openBulkImport("update");
  ...
  const getShortcuts = () => {
    if (creatingItem || editingItem) {
      return { primary: [{ shortcuts: [{ key: "↵", description: creatingItem ? "create" : "save" }, { key: "esc", description: "cancel" }] }], secondary: [] };
    }
    const shortcuts = [];
    if (viewLevel === "environments") shortcuts.push({ key: "n", description: "create environment", disabled: isRestricted }, { key: "u", description: "rename environment", disabled: isRestricted });
    else if (viewLevel === "environment") {
      shortcuts.push({ key: "n", description: "create folder", disabled: isRestricted });
      const item = items[selectedIndex];
      if (item?.type !== "secret") shortcuts.push({ key: "u", description: "rename folder", disabled: isRestricted });
      if (item?.type === "secret") shortcuts.push({ key: "⌥u", description: "update secrets", disabled: isRestricted });
      shortcuts.push({ key: "⌥i", description: "import secrets", disabled: isRestricted }, { key: "esc", description: "back" });
    } else {
      shortcuts.push({ key: "⌥u", description: "update secrets", disabled: isRestricted }, { key: "⌥i", description: "import secrets", disabled: isRestricted });
    }
  ...
  return (
    <box flexDirection="column" width={width} height={height} backgroundColor={THEME_COLORS.background}>
      <box flexDirection="column" justifyContent="center" alignItems="center" flexGrow={1} backgroundColor={THEME_COLORS.background}>
        <box flexDirection="column" backgroundColor={THEME_COLORS.header} width={70} paddingTop={1} paddingBottom={1} paddingLeft={2} paddingRight={2}>
          {/* Breadcrumb */}
          <box height={1} width={66} flexDirection="row" justifyContent="space-between" alignItems="center" marginBottom={1}>
            <text>
              <span fg={THEME_COLORS.primary}>relic</span>
              <span fg={THEME_COLORS.textDim}> / </span>
              <span fg={THEME_COLORS.text}><strong>{projectName}</strong></span>
              {selectedEnv && <><span fg={THEME_COLORS.textDim}> / </span><span fg={THEME_COLORS.secondary}>{selectedEnv.name}</span></>}
              {selectedFolder && <><span fg={THEME_COLORS.textDim}> / </span><span fg={THEME_COLORS.accent}>{selectedFolder.name}</span></>}
            </text>
            <text><span fg={STATUS_COLORS[projectStatus]}>{projectStatus === "owned" ? "●" : projectStatus === "shared" ? "◉" : "○"} {projectStatus}</span></text>
          </box>

          {/* Item list */}
          <box flexDirection="column" width={66} height={items.length === 0 && !creatingItem ? 1 : Math.min(items.length + (creatingItem ? 1 : 0) + (confirmingDelete ? 1 : 0), PAGE_SIZE + (confirmingDelete ? 1 : 0))}>
                ...
                {items.slice(scrollOffset, scrollOffset + PAGE_SIZE).map((item, index) => {
                  ...
                  return (
                    <box key={item.id} flexDirection="column">
                      {isEditing ? (
                        <InlineInput active={true} initialValue={item.name} onSubmit={handleRenameItem} onCancel={() => setEditingItem(null)} maxWidth={50} maxLength={30} width={66} icon="[~]" iconColor={THEME_COLORS.accent} />
                      ) : (
                        <box height={1} width={66}>
                          <text>
                            <span fg={isSelected ? THEME_COLORS.primary : THEME_COLORS.textDim}>{isSelected && canEnter ? "› " : "  "}</span>
                            ...
                            {item.type === "secret" && (() => {
                              ...
                              const maxLen = 66 - 6 - item.name.length - 2 - secretType.length - 3;
                              const display = value.length > maxLen ? `${value.slice(0, maxLen - 3)}...` : value;
                              return <><span fg={THEME_COLORS.textDim}>: </span><span fg={THEME_COLORS.secondary}>{secretType}</span><span fg={THEME_COLORS.textDim}> = </span><span fg={THEME_COLORS.accent}>{display}</span></>;
                            })()}
                          </text>
                        </box>
                      )}
                      <DeleteConfirmation itemType={viewLevel === "environments" ? "environment" : item.type === "folder" ? "folder" : "secret"} itemName={item.name} visible={confirmingDelete?.id === item.id} />
                    </box>
                  );
                })}
                {creatingItem && (
                  <InlineInput active={true} onSubmit={handleCreateItem} onCancel={() => setCreatingItem(null)} maxWidth={50} maxLength={30} width={66} placeholder={creatingItem === "env" ? "e.g. production, staging" : "e.g. database, auth"} />
                )}
              </>
            )}
          </box>
          ...
          <box flexDirection="column" marginTop={1}>
            {viewLevel === "environments" && logs.length > 0 && (
              <box height={1} width={66}>
                <text fg={THEME_COLORS.textDim}>Latest Pulse: <span fg={THEME_COLORS.textMuted}>{logs[0].user} {logs[0].action.replace(".", " ")}</span></text>
              </box>
            )}
            <box height={1} width={66} flexDirection="row" justifyContent="space-between">
              ...
              <text fg={THEME_COLORS.textDim}>
                ...
                {viewLevel === "environment" && `${folders.filter((f) => f.environmentId === selectedEnvId).length} folders · ${secrets.filter((s) => s.environmentId === selectedEnvId && !s.folderId).length} secrets`}
                {viewLevel === "folder" && `${secrets.filter((s) => s.folderId === selectedFolderId).length} secrets`}
              </text>
            </box>
          </box>
          ...
          {(activeModal === "none" || activeModal === "commandPalette") && (
            <box marginTop={1}><GuideBar groups={getShortcuts()} inline={true} customWidth={66} minimal={true} showHelp={true} /></box>
          )}
        </box>
      </box>
      ...
      <BulkImportModal
        ...
        cursorVisible={true}
        mode={bulkImportMode}
        onClose={() => { setActiveModal("none"); bulkImportInput.reset(); }}
      />
    </box>
  );
}
