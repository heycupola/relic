...
![Relic TUI](/assets/tui-overview.png)
...
<Tabs>
  <Tab title="curl">```bash curl -fsSL https://relic.so/install | bash ```</Tab>
  <Tab title="Homebrew">
    ```bash brew install heycupola/tap/relic ``` Available on macOS and Linux.
  </Tab>
  <Tab title="npm">```bash npm install -g relic ```</Tab>
  <Tab title="Bun">```bash bun add -g relic ```</Tab>
</Tabs>
...
<Steps>
  <Step title="Sign in">
    ...
    ![TUI login](/assets/tui-login.png)
